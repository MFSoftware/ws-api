...
class WebSocketAPI {
  constructor(opts = {}) {
    ...
    this.sendAll = message => {
      ...
      for (let i = 0; i < this.clients.length; i++) this.clients[i]['ws'].send(msg);
    };
  }
  ...
  on(event, opts, ...callbacks) {
    ...
    if (opts.schema) if (opts.required) for (let i = 0; i < opts.required.length; i++) cbObject.schema['required'].push(opts.required[i]);
